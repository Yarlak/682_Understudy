...
public class AgentSoccer : Agent
{
    ...
    public override void CollectObservations()
    {
		...
		if (true)
		{
			float master_action = (float)master.GetComponent<AgentSoccer>().theAction;
			List <float> theWhat = new List<float>();
			theWhat.Add(master_action);
			//AddVectorObs(transform.position);
		}
		//List<float> temp_it = rayPer.Perceive(rayDistance, rayAngles, detectableObjects, 0f, 0f);
		//print(temp_it.Count);
