...
public class dubs3_test_Agent : Agent {
	...
	public override void CollectObservations()
	{
		...
		if (is_player)
		{
			AddVectorObs(master.GetComponent<coach_test_Agent>().team_commands[p_name + "_1"]);
			AddVectorObs(master.GetComponent<coach_test_Agent>().team_commands[p_name + "_2"]);
		}else
		{
	...
	public override void AgentAction(float[] vectorAction, string textAction)
	{
		...
		// Getting closer
		if (Target.GetComponent<dubs3_test_reward>().is_active == 1)
		{
			dist1 = distanceToTarget/10;
		}
		
		if (Target1.GetComponent<dubs3_test_reward>().is_active == 1)
		{
		...
		if (is_player)
		{
			master.GetComponent<coach_test_Agent>().AddReward(dist_reward);
		}else
		{
